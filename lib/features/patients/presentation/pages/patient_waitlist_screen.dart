...
class PatientsWaitlistScreen extends StatefulWidget {
  const PatientsWaitlistScreen({super.key});

  @override
  State<PatientsWaitlistScreen> createState() => _PatientsWaitlistScreenState();
}

class _PatientsWaitlistScreenState extends State<PatientsWaitlistScreen> {
  String _searchQuery = '';
  ...
  void initState() {
    super.initState();
    context.read<PatientBloc>().add(const PatientFetchAll(shouldRefresh: false));
      ...
      body: BlocConsumer<PatientBloc, PatientState>(
        listener: (context, state) {
          switch (state.actionStatus) {
            ...
            case (Status.failure):
              LoadingDialog.close();
              _showErrorSnackBar("خطأ بالإتصال، يرجى المحاولة مجددًا");
              if (state.failure != null) log(state.failure!.message);
