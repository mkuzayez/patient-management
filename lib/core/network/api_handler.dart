...
mixin ApiHandler {
  Future<Either<Failure, T>> handleApiCall<T>({
    ...
    try {
      ...
      final isSuccess = response.statusCode == 200 || response.statusCode == 201 || response.statusCode == 202;

      if (isSuccess) {
        if (jsonConvert != null) return Right(jsonConvert(response.data));
        if (returnData) return Right(response.data);
